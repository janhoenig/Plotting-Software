...
parser.add_argument('-c','--filename', help='This program makes plots from oszi data!',\
                    default='D:/data/NitroStrip/NitroStrip/CVLocations.txt')
...
for file in content:
    ...
    # for sep, item in enumerate(data[:,2]):
    #     if 1.0/(abs(item)*abs(item)) >= 7e20:
    #         print()
    #         print(nameString)
    #         sys.exit(0)
    ...
    if nameString2 == 'Mini2':
        ax.plot(data[:,0],1.0/(data[:,2]*data[:,2]),color='black',linewidth=3)
    elif nameString2 == 'Mini8':
        ax.plot(data[:,0],1.0/(data[:,2]*data[:,2]),color='red',linewidth=3)
    elif nameString2 == 'Mini14':
        ax.plot(data[:,0], 1.0/(data[:,2]*data[:,2]), color='blue', linewidth=3)
    elif nameString2 == 'Mini20':
        ax.plot(data[:,0], 1.0/(data[:,2]*data[:,2]), color='green', linewidth=3)
    else:
...
ax.set_ylim(0,800e18)
ax.set_xlim(0,-500.0)

plt.xlabel(u"Voltage in V",fontsize=18)
plt.ylabel(u"1/C^2 in 1/F^2",fontsize=18)
...
fig.savefig(dataString+"/" + "CVCurves"+".png")
fig.savefig(dataString+"/" + "CVCurves"+".pdf")
# plt.show()
